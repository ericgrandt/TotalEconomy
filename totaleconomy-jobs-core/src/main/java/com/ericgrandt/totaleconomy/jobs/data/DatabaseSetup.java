...
public class DatabaseSetup {
    public static void init(Connection conn) throws SQLException {
        createPlayerActiveJobTable(conn);
        createJobExperienceTable(conn);
    }

    private static void createPlayerActiveJobTable(Connection conn) throws SQLException {
        String sql = """
            CREATE TABLE IF NOT EXISTS te_player_active_job (
                id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
                player_id VARCHAR(36) NOT NULL,
                job_id VARCHAR(50) NOT NULL,
                joined_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
                UNIQUE KEY uk_te_player_active_job_player (player_id)
            )
            """;
